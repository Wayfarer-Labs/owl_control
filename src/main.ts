...
function createTray() {
  try {
    ...
    // Path to the owl emoji SVG packaged by webpack
    const iconPath = path.join(__dirname, 'assets', 'owl-emoji.svg');

    // Load the image using Electron's nativeImage to ensure cross-platform support
